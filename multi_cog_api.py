...
# Render-safe path.
# Default expects: data/output/{dataset_name}/*.tif
...
# Classified style for temperature. Adjust colors/breaks here if you want exact QGIS QML matching.
TEMPERATURE_CLASSES = [
    {"label": "< 28 °C", "min": None, "max": 28.0, "color": [49, 54, 149, 210]},
    {"label": "28–30 °C", "min": 28.0, "max": 30.0, "color": [69, 117, 180, 210]},
    {"label": "30–32 °C", "min": 30.0, "max": 32.0, "color": [116, 173, 209, 210]},
    {"label": "32–34 °C", "min": 32.0, "max": 34.0, "color": [171, 217, 233, 210]},
    {"label": "34–36 °C", "min": 34.0, "max": 36.0, "color": [255, 255, 191, 215]},
    {"label": "36–38 °C", "min": 36.0, "max": 38.0, "color": [253, 174, 97, 220]},
    {"label": "38–40 °C", "min": 38.0, "max": 40.0, "color": [244, 109, 67, 225]},
    {"label": "> 40 °C", "min": 40.0, "max": None, "color": [165, 0, 38, 230]},
]

# Optional classified style for rainfall. Default rainfall style remains continuous.
RAINFALL_CLASSES = [
    {"label": "0 mm", "min": None, "max": 0.01, "color": [255, 255, 255, 0]},
    {"label": "0–20 mm", "min": 0.01, "max": 20.0, "color": [198, 219, 239, 190]},
    {"label": "20–50 mm", "min": 20.0, "max": 50.0, "color": [107, 174, 214, 200]},
    {"label": "50–100 mm", "min": 50.0, "max": 100.0, "color": [49, 130, 189, 210]},
    {"label": "100–150 mm", "min": 100.0, "max": 150.0, "color": [254, 217, 118, 220]},
    {"label": "150–200 mm", "min": 150.0, "max": 200.0, "color": [253, 141, 60, 225]},
    {"label": "> 200 mm", "min": 200.0, "max": None, "color": [189, 0, 38, 230]},
]

VALID_RANGE_MODES = ["dynamic", "standard", "max", "p95", "manual"]
VALID_STYLES = ["auto", "continuous", "classified"]
...
def resolve_style(dataset: str, style: Optional[str]) -> str:
    ...
    style_value = (style or "auto").lower().strip()

    if style_value not in VALID_STYLES:
        raise HTTPException(status_code=400, detail="Invalid style. Use auto, continuous, or classified.")
...
def resolve_range_mode(range_mode: Optional[str]) -> str:
    mode = (range_mode or "dynamic").lower().strip()
    if mode not in VALID_RANGE_MODES:
        raise HTTPException(
            status_code=400,
            detail="Invalid range_mode. Use dynamic, standard, max, p95, or manual.",
        )
    return mode


def get_classes_for_dataset(dataset: str) -> List[Dict]:
    variable = get_dataset_config(dataset)["variable"]
    if variable == "temperature":
...
def list_dataset_tifs(dataset: str) -> List[Path]:
    ...
    if not tif_files:
        raise HTTPException(status_code=404, detail=f"No GeoTIFF or COG files found in {dataset_dir}")
...
def get_available_months(dataset: str) -> List[Dict]:
    ...
    for month in range(1, 13):
        try:
            ...
            months.append({"month": month, "name": MONTH_NAMES[month], "available": True, "filename": path.name, "band_index": bidx})
        except HTTPException:
            months.append({"month": month, "name": MONTH_NAMES[month], "available": False, "filename": None, "band_index": None})
    return months
    ...
    try:
        with rasterio.open(cog_path) as src:
            if bidx < 1 or bidx > src.count:
                raise HTTPException(status_code=400, detail=f"Invalid bidx={bidx}. File has {src.count} band(s).")
            ...
            if data.size == 0:
                raise HTTPException(status_code=500, detail="No valid data found in raster.")
        ...
        raise HTTPException(status_code=500, detail=f"Cannot calculate COG statistics for {cog_path.name}: {str(exc)}")


def get_recommended_rescale(
    dataset: str,
    cog_path: Path,
    bidx: int = 1,
    ...
    if mode == "standard":
        default_min, default_max = config["default_rescale"]
        return float(default_min), float(default_max)

    if variable == "temperature":
        # Used only for continuous temperature rendering and legend fallback.
        ...
        rescale_min = 0.0
        if mode == "max":
            rescale_max = max(stats["max"], 10.0)
        elif mode == "p95":
            rescale_max = max(stats["p95"], 10.0)
        else:
            # dynamic default = 0 to p98. Better contrast than fixed 0-300, but still avoids one extreme pixel.
            rescale_max = max(stats["p98"], 10.0)
...
def parse_rescale(
    ...
    if rescale and rescale.lower() != "auto":
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid rescale value. Use min,max or rescale=auto.")

    if cog_path is not None:
        return get_recommended_rescale(dataset, cog_path, bidx=bidx, range_mode=range_mode)
...
def render_classified_png(image_data, classes: List[Dict]) -> bytes:
    ...
    valid_mask = np.isfinite(raw) & (~data_mask)
    ...
    for item in classes:
        ...
        if min_value is not None:
            class_mask = class_mask & (raw >= float(min_value))
        if max_value is not None:
            class_mask = class_mask & (raw < float(max_value))
...
def get_cog_geographic_bounds(cog) -> Tuple[float, float, float, float]:
    ...
    try:
        if dataset_crs.to_epsg() == 4326:
            return tuple(bounds)
        return transform_bounds(dataset_crs, "EPSG:4326", bounds[0], bounds[1], bounds[2], bounds[3], densify_pts=21)
    except Exception:
        return tuple(bounds)
...
async def home(request: Request):
    return templates.TemplateResponse(name="index.html", request=request, context={"title": APP_TITLE})
...
@app.get("/health")
def health():
    return {"status": "ok", "message": "Multi COG API is running", "version": APP_VERSION, "cog_root": str(COG_ROOT)}
...
def service_info(request: Request):
    ...
    return {
        ...
        "stats_example": f"{base_url}/cog/stats?dataset=rain_ssp585_4160_p95_cog&month=1",
        "dynamic_rainfall_example": (
            f"{base_url}/cog/tiles/WebMercatorQuad/5/25/14.png"
            f"?dataset=rain_ssp585_4160_p95_cog&month=1&style=continuous&range_mode=dynamic&rescale=auto&colormap_name=viridis"
        ),
        "classified_temperature_example": (
            f"{base_url}/cog/tiles/WebMercatorQuad/5/25/14.png"
            f"?dataset=temp_ssp585_4160_p95_cog&month=1&style=classified&range_mode=dynamic&rescale=auto&colormap_name=turbo"
        ),
    }
...
def list_datasets():
    ...
    for dataset_name, config in SUPPORTED_DATASETS.items():
        ...
        tif_count = 0
        if exists:
            tif_count = len(list(dataset_dir.glob("*.tif")) + list(dataset_dir.glob("*.tiff")) + list(dataset_dir.glob("*.TIF")) + list(dataset_dir.glob("*.TIFF")))
        ...
        items.append(
            {
                ...
                "default_range_mode": "dynamic" if config["variable"] == "rainfall" else "dynamic",
                "range_modes": VALID_RANGE_MODES,
                "default_colormap": config["default_colormap"],
                "default_style": default_style,
                "classes": get_classes_for_dataset(dataset_name) if default_style == "classified" else [],
                "folder_exists": exists,
                "file_count": tif_count,
            }
        )

    return {"cog_root": str(COG_ROOT), "count": len(items), "datasets": items}
            ...
            layers.append(
                {
                    ...
                    "default_rescale": config["default_rescale"],
                    "recommended_rescale_mode": "auto",
                    "default_range_mode": range_mode,
                    "default_style": style,
                    "default_colormap": colormap_name,
                    "classes": get_classes_for_dataset(dataset_name) if style == "classified" else [],
                    "tile_url_template": build_tile_url(request, dataset_name, month_item["month"], rescale, colormap_name, style=style, range_mode=range_mode),
                    "example_tile_url": build_example_tile_url(request, dataset_name, month_item["month"], rescale, colormap_name, style=style, range_mode=range_mode),
                    "tilejson_url": build_tilejson_url(request, dataset_name, month_item["month"], rescale, colormap_name, style=style, range_mode=range_mode),
                    "stats_url": f"{get_public_base_url(request)}/cog/stats?dataset={dataset_name}&month={month_item['month']}&range_mode={range_mode}",
                }
            )
...
def dataset_layers(dataset: str, request: Request):
    ...
    return {
        ...
        "months": [
            {
                **month_item,
                "tile_url_template": build_tile_url(request, dataset, month_item["month"], rescale, colormap_name, style=style, range_mode=range_mode) if month_item["available"] else None,
                "example_tile_url": build_example_tile_url(request, dataset, month_item["month"], rescale, colormap_name, style=style, range_mode=range_mode) if month_item["available"] else None,
                "tilejson_url": build_tilejson_url(request, dataset, month_item["month"], rescale, colormap_name, style=style, range_mode=range_mode) if month_item["available"] else None,
                "stats_url": f"{get_public_base_url(request)}/cog/stats?dataset={dataset}&month={month_item['month']}&range_mode={range_mode}" if month_item["available"] else None,
            }
            for month_item in months
        ],
    }
...
def cog_stats(
    ...
    range_mode: str = Query("dynamic", description="dynamic, standard, max, p95, or manual"),
    bidx: Optional[int] = Query(None, ge=1),
):
    validate_dataset(dataset)
    resolved_range_mode = resolve_range_mode(range_mode)
    ...
    rescale_min, rescale_max = get_recommended_rescale(dataset, cog_path, bidx=resolved_bidx, range_mode=resolved_range_mode)
    config = SUPPORTED_DATASETS[dataset]
    style = resolve_style(dataset, "auto")
...
def cog_info(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Cannot read COG info: {str(exc)}")
...
def tilejson(
    ...
    cog_path = find_month_cog(dataset, month)
    resolved_bidx = resolve_band_index(cog_path, month, requested_bidx=bidx)
    rescale_min, rescale_max = parse_rescale(rescale, dataset, cog_path=cog_path, bidx=resolved_bidx, range_mode=resolved_range_mode)
    rescale_text = format_rescale(rescale_min, rescale_max)

    tile_url = build_tile_url(request, dataset, month, rescale_text, colormap_name, bidx=resolved_bidx, style=resolved_style, range_mode=resolved_range_mode)
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Cannot create TileJSON: {str(exc)}")
    ...
    try:
        with COGReader(str(cog_path)) as cog:
            ...
            if resolved_style == "classified":
                ...
                if not classes:
                    raise HTTPException(status_code=400, detail=f"No classified style configured for dataset={dataset}")
...
def preview_url(
    ...
    return {
        ...
        "tile_url_template": build_tile_url(request, dataset, month, rescale_text, colormap_name, bidx=resolved_bidx, style=resolved_style, range_mode=resolved_range_mode),
        "example_tile_url": build_example_tile_url(request, dataset, month, rescale_text, colormap_name, bidx=resolved_bidx, style=resolved_style, range_mode=resolved_range_mode),
        "tilejson_url": build_tilejson_url(request, dataset, month, rescale_text, colormap_name, bidx=resolved_bidx, style=resolved_style, range_mode=resolved_range_mode),
        "stats_url": f"{get_public_base_url(request)}/cog/stats?dataset={dataset}&month={month}&range_mode={resolved_range_mode}&bidx={resolved_bidx}",
    }
